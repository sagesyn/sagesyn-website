  ...
  return (
    <div
      className={`rounded-2xl border p-6 transition-all sm:p-8 ${
        isActive ? `${colors.border} ${colors.bg}` : "border-border bg-surface"
      }`}
    >
      {/* Header */}
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted text-sm">{phase.quarter}</p>
            </div>
          </div>
        </div>
        {isActive && (
          <span className="bg-brand-cyan/10 text-brand-cyan inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted mt-2 text-sm">{phase.description}</p>
      </div>
